...
void demoParticle::reset() {
	...
	vel.x = ofRandom(-3.9, 7.9);
	vel.y = ofRandom(-3.9, 7.9);
...
//------------------------------------------------------------------
void demoParticle::update(ofPoint(kinX),ofPoint(kinY)) {

	//1 - APPLY THE FORCES BASED ON WHICH MODE WE ARE IN 

	if (mode == PARTICLE_MODE_ATTRACT) {
		ofPoint attractPt(kinX);
...
void demoParticle::draw() {
	...
	ofDrawCircle(pos.x, pos.y, scale * 34.0);
}
